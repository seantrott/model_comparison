...
If you want to see the slides from a presentation of this tutorial, see [here](https://seantrott.github.io/model_comparison/presentation.html).

# Logistics

First, logistics. We'll be using four libraries: `library(tidyverse)`, `library(lme4)`,  `library(lmtest)`, and `library(ggridges)`. If you don't have them installed already, use `install.packages("tidyverse")`, `install.packages("lme4")`, `install.packages("lmtest")`, and `install.packages("ggridges")` to install them.
...
That's a very low p-value! This means that we're very unlikely to have achieved a value this large under the null distribution. We can double-check our work using the `lrtest` function:
...
While we *could* just average these predictions in an unweighted way, we know that these models have pretty different fits. Thus, we should be more confident about predictions from $m_1$, which has a normalized probability of `r round(p[1], 2)`, than $m_3$, which has a normalized probability of `r round(p[3], 2)`.
